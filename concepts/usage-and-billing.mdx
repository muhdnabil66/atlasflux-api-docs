...
The model provider reports reasoning tokens as part of its completion-token total. AtlasFlux separates that total into visible `output_tokens` and `reasoning_tokens`, then bills each token exactly once at the applicable rate. `total_tokens` is `input_tokens + output_tokens + reasoning_tokens`.
